...
class TDLCBirthday {
	...
	private function printBirthdays($showeveryone = false) {
		...
		if (empty($happybday)&&empty($upcoming)) {
			switch ($this->offset) {
				...
				case 1:
					...
					break;
				default:
					printf(__('   Sem aniversariantes, próximo será há %d dias...', 'tdlc-birthdays'), $this->offset);
					break;
			}
		}
		...
		// Display link to user's profile if their birthdate is missing (optional) CRIARENET
	...
	private function happy_bday($list) {
		...
		foreach($list as $buddy) {
			...
			?>
				<li style="min-height:20px; margin-left: 15px;">
					<div style="float:left; padding:0px; margin-left: 15px">
					<a href="<?php echo $bpbuddy->user_url ?>"><?php echo $bpbuddy->avatar_thumb ?></a></div>
					<?php
					echo __('<strong>Feliz Aniversário, ', 'tdlc-birthdays').$bpbuddy->user_link;
		...
		foreach($list as $buddy) {
			...
			?>
				<li>
					...
					<div style="display:block;margin-left:15px;padding:0px 0 0px 0">
					<?php 
					if ($this->hideage==1 || $buddy->getAge()==0) 
						echo $bpbuddy->user_link.sprintf(__(' %1$d/%2$d', 'tdlc-birthdays'), date("j",$buddy->birthdate), date("m",$buddy->birthdate));
					else
						echo $bpbuddy->user_link.sprintf(__(' %1$d/%2$d -  %3$d anos', 'tdlc-birthdays'), date("j",$buddy->birthdate), date("m",$buddy->birthdate), $buddy->getAge());
					?>
					</div>
				</li>
			<?php
